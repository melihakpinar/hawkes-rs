...
Parameters = _hawk.UnivariateParameters
Fit = _hawk.UnivariateFit
...
def negative_log_likelihood(
    ...
    """
    return _hawk.univariate_negative_log_likelihood(parameters, times, horizon)
...
def negative_log_likelihood_and_gradient(
    ...
    value, baseline, excitation, decay = (
        _hawk.univariate_negative_log_likelihood_and_gradient(parameters, times, horizon)
    )
...
def simulate(parameters: Parameters, horizon: float, seed: int) -> npt.NDArray[np.float64]:
    ...
    Returns:
        ...
        aliases Rust memory.
    """
    return _hawk.univariate_simulate(parameters, horizon, seed)
...
def fit(times: npt.NDArray[np.float64], horizon: float) -> Fit:
    ...
    """
    return _hawk.univariate_fit(times, horizon)
