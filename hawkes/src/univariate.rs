...
/// pass, which showed up as `hawkes` making 36 passes over the data per fit where
...
/// order. `hawkes/tests/bit_identical_evaluation.rs` enforces it; a change that
...
/// hypothesis that does not survive `hawkes`'s input contract (C8) rather than an error
...
/// `epsilon = 1e-10`, because it treats `lambda*` as a black box. `hawkes` carries the
