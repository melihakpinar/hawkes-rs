...
//! # Sabotage
//!
//! Transposing the excitation in the simulator turned the per-component
//! mean-intensity test red while leaving the pooled total green. Recorded in
//! `docs/verification-log.md`.
//!
//! The guard that `stationary_mean_intensity` checks the spectral radius rather than
//! inferring stationarity from the solve — `I - alpha` can be invertible with radius
//! above 1 — lives in `spectral_radius.rs` (`stationarity_verdicts_on_reducible_matrices`
//! and `irreducible_matrices_are_unaffected`); S53 records the sabotage that showed the
//! copy this file used to carry was redundant with it.
...
fn the_ks_test_rejects_residuals_from_a_transposed_matrix() {
    ...
    for values in &compensators {
        ...
        let n = residuals.len() as f64;
        let statistic = ks_statistic_against_unit_exponential(&mut residuals);
        if statistic > KS_CRITICAL_VALUE_1_PERCENT / n.sqrt() {
            any_rejected = true;
        }
    }
    assert!(
