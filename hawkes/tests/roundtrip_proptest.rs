...
//! [`hawkes::univariate::simulate`] and [`hawkes::univariate::fit`].
//!
...
proptest! {
    ...
    fn recovers_parameters_within_their_standard_errors(
        ...
        // tolerance.
        let at_truth = hawkes::univariate::negative_log_likelihood(&truth, &observation);
        prop_assert!(
...
fn the_fit_actually_optimizes() {
    ...
    let at_start = hawkes::univariate::negative_log_likelihood(&start, &observation);
