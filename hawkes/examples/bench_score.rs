//! Scores `tick`'s returned parameters under `hawkes`'s unpenalized objective, so two
...
use hawkes::multivariate::{Observation, Parameters, negative_log_likelihood};
...
fn main() {
    ...
    for run in &mut side.runs {
        ...
        eprintln!(
            "d={} n={} scored under hawkes nll = {value:.6} (rho={spectral:.6})",
            run.dimension, run.nominal_n
        );
    }
