//! Writes the negative log-likelihood `hawkes` computes for every fixture and parameter
//! point, as an exact bit pattern.
//!
//! This is what the Python bindings are compared against in
//! `hawkes-python/tests/test_bit_identity.py`. Both sides call the same Rust function, so
