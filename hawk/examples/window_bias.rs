...
fn main() {
    ...
    for fraction in DEAD_TIME_FRACTIONS {
        ...
        eprintln!(
            "dead_time={:5.0}% declared_T={declared:10.2} hawk baseline={:.6}",
            fraction * 100.0,
    ...
    let body = format!(
        r#"{{"library":"hawk","experiment":"window_bias","question":"OQ-5","seed":{SEED},"events":{},"observed_horizon":{observed_horizon:?},"last_event":{last_event:?},"true_baseline":{BASELINE:?},"true_excitation":{EXCITATION:?},"true_decay":{DECAY:?},"rows":[{}]}}"#,
        times.len(),
        rows.join(",")
    );
