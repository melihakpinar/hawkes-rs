...
//! `hawkes-python/tests/test_bit_identity.py` proves it: the negative log-likelihood
...
/// Maps a `hawkes::Error` onto a Python exception.
///
...
/// interpreter. The mapping is documented in `python/hawkes/__init__.py`.
...
struct UnivariateParameters {
    inner: hawkes::univariate::Parameters,
}
...
impl UnivariateParameters {
    ...
    fn new(baseline: f64, excitation: f64, decay: f64) -> PyResult<Self> {
        Ok(Self {
            inner: hawkes::univariate::Parameters::new(baseline, excitation, decay)
                .map_err(to_py_error)?,
        })
    }
...
fn univariate_negative_log_likelihood(
    ...
    Ok(hawkes::univariate::negative_log_likelihood(
        &parameters.inner,
        &observation,
    ))
}
...
fn univariate_negative_log_likelihood_and_gradient(
    ...
    let observation = hawkes::univariate::Observation::new(&owned, horizon).map_err(to_py_error)?;
    let (value, gradient) =
        hawkes::univariate::negative_log_likelihood_and_gradient(&parameters.inner, &observation);
    Ok((
        value,
        gradient.baseline,
        gradient.excitation,
...
fn univariate_simulate<'py>(
    ...
    let times =
        hawkes::univariate::simulate(&parameters.inner, horizon, &mut rng).map_err(to_py_error)?;
...
fn univariate_fit(times: PyReadonlyArray1<'_, f64>, horizon: f64) -> PyResult<UnivariateFit> {
    let owned = vector_from(&times);
    let observation = hawkes::univariate::Observation::new(&owned, horizon).map_err(to_py_error)?;
    let fit = hawkes::univariate::fit(&observation).map_err(to_py_error)?;
    Ok(UnivariateFit {
        parameters: UnivariateParameters {
...
struct MultivariateParameters {
    inner: hawkes::multivariate::Parameters,
}
...
impl MultivariateParameters {
    ...
    fn new(
        ...
        Ok(Self {
            inner: hawkes::multivariate::Parameters::new(baseline, flat, decay)
                .map_err(to_py_error)?,
        })
    }
...
fn multivariate_negative_log_likelihood(
    ...
    let observation =
        hawkes::multivariate::Observation::new(&owned, horizon).map_err(to_py_error)?;
    hawkes::multivariate::negative_log_likelihood(&parameters.inner, &observation)
        .map_err(to_py_error)
}
...
fn multivariate_negative_log_likelihood_and_gradient<'py>(
    ...
    let observation =
        hawkes::multivariate::Observation::new(&owned, horizon).map_err(to_py_error)?;
    let (value, gradient) =
        hawkes::multivariate::negative_log_likelihood_and_gradient(&parameters.inner, &observation)
            .map_err(to_py_error)?;
...
fn multivariate_compensator_at_events<'py>(
    ...
    let observation =
        hawkes::multivariate::Observation::new(&owned, horizon).map_err(to_py_error)?;
    Ok(
        hawkes::multivariate::compensator_at_events(&parameters.inner, &observation)
            .map_err(to_py_error)?
            .into_iter()
...
fn multivariate_simulate<'py>(
    ...
    let mut rng = ChaCha8Rng::seed_from_u64(seed);
    let events = hawkes::multivariate::simulate(&parameters.inner, horizon, &mut rng)
        .map_err(to_py_error)?;
    Ok(events
        .into_iter()
        .map(|component| PyArray1::from_vec(py, component))
...
fn multivariate_fit(events: &Bound<'_, PyAny>, horizon: f64) -> PyResult<MultivariateFit> {
    ...
    let observation =
        hawkes::multivariate::Observation::new(&owned, horizon).map_err(to_py_error)?;
    let fit = hawkes::multivariate::fit(&observation).map_err(to_py_error)?;
    Ok(MultivariateFit {
        parameters: MultivariateParameters {
            inner: fit.parameters,
        },
...
#[pymodule]
fn _hawkes(module: &Bound<'_, PyModule>) -> PyResult<()> {
